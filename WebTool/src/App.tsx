...
async function downloadModPackage(pack: ItemPackDefinition) {
  const zip = new JSZip()
  const folderName = pack.name.replace(/\s+/g, '_').toLowerCase() || 'custom_items'
  ...
  root.file('package.json', JSON.stringify({
    name: folderName,
    version: '1.0.0',
    description: `Generated by ItemGen Web Tool — ${pack.name}`,
    author: 'ItemGen',
    license: 'MIT',
    main: '',
    sptVersion: '4.0.13',
  ...
  root.folder('config')?.file('config.json', JSON.stringify({ enabled: true, debug: false }, null, 2))
  root.folder('items')?.file(`${folderName}.json`, buildExportJson(pack))
...
export default function App() {
  ...
  const importJson = useCallback((file: File) => {
    ...
    reader.onload = () => {
      try {
        const imported = JSON.parse(reader.result as string) as ItemPackDefinition
        ...
        alert('Invalid JSON file: ' + (e as Error).message)
      }
    }
  ...
  const exportPackage = useCallback(() => downloadModPackage(pack), [pack])
      ...
      <header className="bg-tarkov-surface border-b border-tarkov-border p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button className="btn-primary text-sm flex items-center gap-1.5" onClick={exportPackage}>
            <Download size={14} /> Download Mod Package
          </button>
          <input
            type="file"

...
public class ItemLoader(ISptLogger<ItemLoader> logger, ModHelper modHelper)
{
    ...
    public List<LoadedPack> LoadAllPacks()
    {
        ...
        if (!Directory.Exists(itemsDir))
        {
            ...
            logger.LogWithColor("[ItemGen] Created items/ directory. Place item pack JSON files here.", Color.Yellow);
            return results;
        }
    ...
    private LoadedPack? TryLoadPackFile(string jsonFilePath, string packFolder)
    {
        ...
        try
        {
            ...
            if (pack == null)
            {
                logger.LogWithColor($"[ItemGen] Failed to parse '{fileName}': JSON deserialized to null.", Color.Red);
                return null;
            }

            if (!pack.Enabled)
            {
                logger.LogWithColor($"[ItemGen] Skipping disabled pack '{fileName}'", Color.Yellow);
                return null;
            }
            ...
            if (errors.Count > 0)
            {
                logger.LogWithColor($"[ItemGen] Validation errors in '{fileName}':", Color.Red);
                foreach (var error in errors)
                    logger.LogWithColor($"  - {error}", Color.Red);
                return null;
            }

            logger.LogWithColor($"[ItemGen] Loaded pack '{pack.Name}' from '{fileName}' ({pack.QuestItems.Count} quest items, {pack.Keys.Count} keys)", Color.Green);
            return new LoadedPack(pack, jsonFilePath, packFolder);
        }
        catch (JsonException ex)
        {
            logger.LogWithColor($"[ItemGen] JSON parse error in '{fileName}': {ex.Message}", Color.Red);
            return null;
        }
        catch (Exception ex)
        {
            logger.LogWithColor($"[ItemGen] Error loading '{fileName}': {ex.Message}", Color.Red);
            return null;
        }
    }
}
